...
public class BoardDAO {
	...
	public List selectAllArticles(Map pagingMap) { //반환형은 리스트형
		List articlesList = new ArrayList();
		int section = (Integer)pagingMap.get("section");  //1
		int pageNum = (Integer)pagingMap.get("pageNum");  //1
		...
		try {
			...
			String query = 
					...
					//레벨이 0보다 크면 (2이상일땐 자식) 앞과 뒤를 concat으로 연결함   repeat은 반복문자열                                                                                   레벨은 생성한 것이기때문에 result에 넣음
					 ...
					 + " limit ?,10";
					 // "(section-1)*100 +(pageNum-1)*10 , 10";
					/*
					 * "selcet * from ( " + "select ROWNUM as recNum, " + "LVL," + "articleNO," +
					 * "parentNO, title, id, writeDate from (select level as LVL, " +
					 * " articleNI, parentNO, title, id, writeDate from t_board"
